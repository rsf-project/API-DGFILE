...
func Add_user(c echo.Context) error {
	...
	if controller.Is_username_exist(model.Username) {
...
func Edit_user(c echo.Context) error {
	...
	syn := "select edit_user('" + model.Uid + "','" + model.Name + "','" + controller.Hash_256(model.Password) + "','" + model.Phone + "','" + model.Email + "','" + strconv.Itoa(model.Space) + "');"
	hasil, err := db.Exec(context.Background(), syn)
	if err != nil {
		res.Message = "Data gagal ditambahkan"
		res.Data = 0
