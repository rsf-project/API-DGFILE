...
func Get_path(Item_id string, User_id string) (string, error) {
	...
	var temp []string
	var result string
	if Item_id == "" || Item_id == "null" || Item_id == "undefined" || Item_id == User_id {
		result = User_id
	} else {
		for {
...
func Get_file_list(c echo.Context) error {
	...
	var item_id []int
	var item int
	syn := "select * from get_file_list('" + model.Uid + "','" + curentpath + "');"
	test, err := db.Query(context.Background(), syn)
	if err != nil {
		utils.LogError(err)
...
func Get_folder_list(c echo.Context) error {
	...
	var item_id []int
	var item int
	syn := "select * from get_folder_list('" + model.Uid + "','" + curentpath + "');"
	test, err := db.Query(context.Background(), syn)
	if err != nil {
		utils.LogError(err)
		return c.JSON(http.StatusBadRequest, err)
...
func Get_all_item_list(c echo.Context) error {
	...
	var item_id []int
	var item int
	syn := "select * from Get_all_item_list('" + model.Uid + "','" + curentpath + "');"
	test, err := db.Query(context.Background(), syn)
	if err != nil {
		utils.LogError(err)
		return c.JSON(http.StatusBadRequest, err)
	...
	count := "select * from get_file_count('" + model.Uid + "','" + curentpath + "');"
