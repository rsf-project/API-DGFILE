...
//func file log db
func Add_folder(item_id string, parent_id string, uid string, foldername string) (string, error) {
	syn := "select * from add_folder('" + item_id + "','" + parent_id + "','" + uid + "','" + foldername + "');"
...
func Add_file(item_id string, parent_id string, uid string, filename string, size int64) (bool, error) {
	result := false
	...
	syn := "select add_file('" + item_id + "','" + parent_id + "','" + uid + "','" + filename + "','" + size2.String() + "');"
	hasil, err := db.Exec(context.Background(), syn)
	if err != nil {
...
func Create_folder(c echo.Context) error {
	...
	c.Bind(&model)
	root := "upload/"
	folder_id, _ := utils.GetUID()
	res := responsegraph.Data{
		Status:  constant.StatusSuccess,
		Message: "Berhasil Create Folder",
		Data:    1,
	}
	curentpath, _ := Get_path(model.Parent_id, model.Uid)
	curentpath += "/"
	utils.LogInfo("ini currentpath : " + curentpath)
	utils.LogInfo("ini uid : " + model.Uid)
	if isexist1(root + model.Uid + curentpath + model.Folder_name) {
		res.Message = "Duplikasi ditemukan, gagal Create Folder"
		res.Data = 0
		return c.JSON(http.StatusAccepted, res)
	}
	err := os.MkdirAll(root+curentpath+folder_id, 0755)
	if err != nil {
	...
	Add_folder(folder_id, model.Parent_id, model.Uid, model.Folder_name)
	return c.JSON(http.StatusAccepted, res)
}
...
func Upload_file(c echo.Context) error {
	...
	for i, _ := range files {
		...
		if Is_enough_space(model.Uid, strconv.Itoa(int(files[i].Size))) == false {
		...
		id_file, _ := utils.GetUID()
		ekstensi := strings.Split(files[i].Filename, ".")
		filename := id_file + "." + ekstensi[(len(ekstensi)-1)]
		//ini proses pembuatan file pada temporary disk
		dst, err1 := os.Create(newfolder + filename)
		//ini memanggil function db
		filenamedb := verify_duplicate_file(model.Parent_id, files[i].Filename)
		Add_file(id_file, model.Parent_id, model.Uid, filenamedb, files[i].Size)
		defer dst.Close()
		if err1 != nil {
			utils.LogError(err1)
		}
...
func Upload_folder(c echo.Context) error {
	var model user.Items
	c.Bind(&model)
	curentpath, _ := Get_path(model.Item_id, model.Uid)
	root := "upload/"
	//cek jika directory root belum ada
	if isexist1(root) == false {
		err := os.MkdirAll(root, 0755)
	...
	for i, _ := range files {
		...
		for i := 0; i < len(ah); i++ {
			id_folder, _ := utils.GetUID()
			if i == 0 {
				temp_parent = model.Parent_id
			} else {
				temp_parent = id_folder //.
			}
			//pengecekan directory
			result, _ := duplicate_item(temp_parent, ah[i])
			if result == false {
				//pemanggilan function db
				Add_folder(id_folder, temp_parent, model.Uid, ah[i])
			}
			folder += id_folder + "/"
		}
		...
		dst, err1 := os.Create(newfolder + filename)
		//pemanggilan function db
		filenamedb := verify_duplicate_file(temp_parent, files[i].Filename)
		Add_file(id_file, temp_parent, model.Uid, filenamedb, files[i].Size)
		defer dst.Close()
		if err1 != nil {
			utils.LogError(err1)
		}
