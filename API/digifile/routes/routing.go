...
func GetRoutes() *echo.Echo {
	...
	r_user.PUT("/recovery_trash", user.Recovery_trash)                        //done
	...
	r_user.GET("/get_trash_file_list/:uid", user.Get_trash_file_list)         //done
	r_user.GET("/get_trash_folder_list/:uid", user.Get_trash_folder_list)     //done
	r_user.GET("/search_user", user.Search_user)                              //done, tapi cek bagian username/id
	r_user.POST("/upload_file", user.Upload_file)                             //done, tapi cek bagian penamaan
	r_user.POST("/upload_folder", user.Upload_folder)                         //done, tapi cek bagian penamaan
	r_user.POST("/create_folder", user.Create_folder)                         //done, tapi cek bagian currentpath/id
	...
	r_user.GET("/get_information_storage/:uid", user.Get_information_storage) //done

	//route untuk owner
	r_owner := e.Group("/owner")
	r_owner.POST("/add_user", owner.Add_user)                          //done
	r_owner.POST("/edit_user", owner.Edit_user)                        //done
	r_owner.GET("/get_log_activity", owner.Get_log_activity)           //done
	r_owner.GET("/get_user_log_activity", owner.Get_user_log_activity) //done
	r_owner.GET("/get_user_information", owner.Get_user_information)   //done
	r_owner.GET("/search", owner.Search)                               //done
